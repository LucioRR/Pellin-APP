...
// ── Componente de impresión (siempre en el DOM, oculto hasta print) ────────
function PrintRemito({ remito, negocioNombre, items }) {
    ...
    <div id="print-remito">
        ...
        <tbody>
          {items.map((it, i) => (
            <tr key={it.id}>
              ...
              <td style={{ fontWeight: 700 }}>{it.cantidad}</td>
              <td>{it.unidad}</td>
            </tr>
          ))}
        </tbody>
      ...
      {remito.notas && (
        <div className="print-notas">
          <strong>Notas:</strong> {remito.notas}
        </div>
      )}
...
export default function Remitos() {
  ...
  const canSave = form.destino.trim() && items.every(it => it.productoId && it.cantidad && Number(it.cantidad) > 0)
    ...
    try {
      ...
      for (const it of items) {
        ...
        if (!prod || Number(prod.stock_actual) < Number(it.cantidad)) {
          toast(`Stock insuficiente para ${prod?.nombre || 'producto seleccionado'}`, 'err')
          setSaving(false)
          return
        }
      }
  ...
  const anular = async (motivo) => {
    ...
    try {
      ...
      const { data: its } = await supabase.from('remito_items').select('*').eq('remito_id', anularModal.id)
      ...
      // Revertir stock
      for (const it of (its || [])) {
        const { data: prod } = await supabase.from('productos_terminados').select('stock_actual').eq('id', it.producto_id).single()
        if (prod) {
          await supabase.from('productos_terminados').update({
            stock_actual: Math.round((Number(prod.stock_actual) + Number(it.cantidad)) * 100) / 100
  ...
  const imprimir = async (remito) => {
    const { data: its } = await supabase.from('remito_items').select('*').eq('remito_id', remito.id)
  ...
  return (
    <div>
      <PageHeader title="Remitos de Salida" sub={`${remitos.filter(r => !r.anulado).length} remitos activos`}
        action={
      ...
      <Card>
        <div className="table-responsive">
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              <TH>Nº Remito</TH><TH>Fecha</TH><TH>Destino</TH><TH>Items</TH><TH>Registrado por</TH><TH>Estado</TH><TH></TH>
            </tr></thead>
            <tbody>
              {remitos.length === 0 && <EmptyRow cols={7} msg="Sin remitos registrados" />}
              {remitos.map(r => (
                <tr key={r.id} style={{ background: r.anulado ? '#F9F9F7' : 'transparent', opacity: r.anulado ? 0.7 : 1 }}>
                  <TD bold>{r.numero}</TD>
                  ...
                  <td style={{ padding:'10px 14px', borderBottom:'1px solid var(--border)' }}>
                    <div style={{ display:'flex', gap:6 }}>
                      <BtnSm onClick={() => imprimir(r)} title="Imprimir remito">
        ...
        <div className="cards-mobile" style={{ padding:12 }}>
          {remitos.length === 0 && <p style={{ color:'var(--muted)', fontSize:13, textAlign:'center', padding:20 }}>Sin remitos registrados</p>}
          {remitos.map(r => (
            <div key={r.id} className="mobile-card" style={{ opacity: r.anulado ? 0.7 : 1 }}>
              ...
              <div className="mobile-card-row">
                ...
                <span className="mobile-card-val">{r.total_items} items</span>
              </div>
        ...
        <Modal title="Nuevo Remito de Salida" wide onClose={() => setModal(false)}>
          <Grid2>
            <FG label="Destino" required>
              <Inp value={form.destino} onChange={e => setForm(f => ({ ...f, destino: e.target.value }))}
                placeholder="ej: Local centro, Gondola A..." />
            </FG>
            <FG label="Fecha">
              <Inp type="date" value={form.fecha} onChange={e => setForm(f => ({ ...f, fecha: e.target.value }))} />
            </FG>
          </Grid2>
          ...
          <div style={{ borderTop:'1px solid var(--border)', paddingTop:14, marginTop:4 }}>
            ...
            {items.map((it, i) => (
              <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:8, marginBottom:10, alignItems:'flex-end' }}>
                ...
                <div>
                  {i === 0 && <div style={{ fontSize:11, color:'var(--muted)', marginBottom:4 }}>Cantidad{it.unidad ? ` (${it.unidad})` : ''}</div>}
                  <Inp type="number" value={it.cantidad}
                    onChange={e => updateItem(i, 'cantidad', e.target.value)}
                    placeholder="0"
                    max={it.productoId ? productos.find(p => p.id === it.productoId)?.stock_actual : undefined}
                  />
                  {it.productoId && it.cantidad && (() => {
                    const prod = productos.find(p => p.id === it.productoId)
                    const insuf = prod && Number(it.cantidad) > Number(prod.stock_actual)
                    return insuf ? <div style={{ fontSize:11, color:'#BF3030', marginTop:3 }}>⚠ Stock disponible: {prod.stock_actual}</div> : null
                  })()}
                </div>
                <button onClick={() => setItems(s => s.filter((_, j) => j !== i))} disabled={items.length === 1}
            ...
            <button onClick={() => setItems(s => [...s, { productoId:'', productoNombre:'', cantidad:'', unidad:'' }])}
          ...
          <FG label="Notas (opcional)" style={{ marginTop:12 }}>
            <Inp value={form.notas} onChange={e => setForm(f => ({ ...f, notas: e.target.value }))}
      ...
      {/* Modal anular */}
      {anularModal && (
        <AnularModal titulo={`Remito ${anularModal.numero}`} onConfirm={anular} onClose={() => setAnularModal(null)} loading={saving} />
      )}

      {/* Contenido de impresión (siempre en el DOM) */}
      <PrintRemito remito={printData.remito} items={printData.items} negocioNombre={negocioNombre} />
    </div>
  )
}
